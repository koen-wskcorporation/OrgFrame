...
async function getNextNavSortIndex(orgId: string, parentId: string | null) {
  const supabase = await createSupabaseServer();
  let query = supabase
    .schema("site").from("blocks")
    .select("order_index")
    .eq("row_kind", "structure_item")
    .eq("org_id", orgId)
...
async function loadBlocks(orgPageId: string, context: BlockContext) {
  ...
  const { data, error } = await supabase
    .schema("site").from("blocks")
    .select(blockSelect)
    .eq("row_kind", "page_block")
    .eq("org_page_id", orgPageId)
...
async function loadPageBySlug(orgId: string, pageSlug: string, includeUnpublished: boolean) {
  const supabase = await createSupabaseServer();
  let query = supabase.schema("site").from("pages").select(pageSelect).eq("org_id", orgId).eq("slug", pageSlug);
...
export async function getOrgPageById(orgId: string, pageId: string): Promise<OrgSitePage | null> {
  const supabase = await createSupabaseServer();
  const { data, error } = await supabase.schema("site").from("pages").select(pageSelect).eq("org_id", orgId).eq("id", pageId).maybeSingle();
...
export async function ensureOrgPageExists({
  ...
  const { error: insertBlocksError } = await supabase.schema("site").from("blocks").insert(
...
export async function saveOrgPageAndBlocks({
  ...
  const { error: deleteError } = await supabase
    ...
    .delete()
    .eq("row_kind", "page_block")
    .eq("org_page_id", existing.page.id);

  if (deleteError) {
    throw new Error(`Failed to replace org page blocks: ${deleteError.message}`);
  }

  const { error: insertError } = await supabase.schema("site").from("blocks").insert(
...
export async function deleteOrgPageById(orgId: string, pageId: string): Promise<boolean> {
  ...
  const { data, error } = await supabase.schema("site").from("pages").delete().eq("org_id", orgId).eq("id", pageId).select("id").maybeSingle();
...
export async function reorderOrgPages(orgId: string, orderedPageIds: string[]): Promise<OrgManagePage[]> {
  ...
  for (const [index, pageId] of orderedPageIds.entries()) {
    const { error } = await supabase.schema("site").from("pages").update({ sort_index: index + offset }).eq("org_id", orgId).eq("id", pageId);
  ...
  for (const [index, pageId] of orderedPageIds.entries()) {
    const { error } = await supabase.schema("site").from("pages").update({ sort_index: index }).eq("org_id", orgId).eq("id", pageId);
