...
import { addFormToPageAction, getFormSharingDataAction, type FormSharingData } from "@/src/features/forms/actions";
...
function asPagePath(pageSlug: string) {
  if (pageSlug === "home") {
    return "/";
  }
...
export function FormSharingPanel({ open, onClose, orgSlug, formId, formSlug }: FormSharingPanelProps) {
  ...
  const formPath = `/register/${activeFormSlug}`;
              ...
              {includedPages.map((page) => (
                <div className="flex items-center justify-between rounded-control border bg-surface px-3 py-2" key={page.id}>
                  <div className="min-w-0">
                    ...
                    <p className="truncate text-xs text-text-muted">{asPagePath(page.slug)}</p>
                  </div>
                  <div className="ml-3 flex items-center gap-2">
                    {page.includeCount > 1 ? <span className="text-xs text-text-muted">{page.includeCount} blocks</span> : null}
                    <Link className="text-xs font-semibold text-accent hover:underline" href={asPagePath(page.slug)} rel="noopener noreferrer" target="_blank">
                      Open
                    </Link>
                  </div>
                </div>
