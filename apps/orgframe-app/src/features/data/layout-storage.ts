...
import { emptyLayout, normalizeLayout, type DataCenterLayout } from "@/src/features/data/layout";
import type { DashboardPresetDef, ResolvedDataSource } from "@/src/features/data/registry/types";
import type { DataCenterWidgetInstance } from "@/src/features/data/layout";

export async function loadDataCenterLayout(input: { orgId: string; sourceKey: string }): Promise<DataCenterLayout> {
  const supabase = await createSupabaseServer();
  const { data } = await supabase
    .schema("orgs")
    .from("org_data_layouts")
    .select("config_json")
...
export async function saveDataCenterLayout(input: {
  ...
  if (error) {
    throw new Error(`Unable to save data layout: ${error.message}`);
  }
  return normalized;
}
