...
} from "@/src/features/programs/schedule/db/queries";
import { generateOccurrencesForRule, zonedLocalToUtc } from "@/src/features/programs/schedule/rule-engine";
import type {
  ProgramOccurrence,
...
async function refreshScheduleData(orgSlug: string, programId: string) {
  ...
  revalidatePath(`/${orgSlug}/manage/programs/${programId}`);
  revalidatePath(`/${orgSlug}/manage/programs/${programId}/schedule`);
  return readModel;
}
