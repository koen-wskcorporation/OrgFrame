...
type AccessResult<TData = undefined> =
  | {
      ok: true;
      data: TData;
    }
...
function asFailure(code: AccessErrorCode, error: string): AccessResult<never> {
  return {
    ok: false,
    code,
    error
...
export async function getAccountsAccessPageData(orgSlug: string): Promise<AccountsAccessPageData> {
  const orgContext = await requireAccessContext(orgSlug);
...
export async function inviteUserToOrgAction(input: {
  ...
  email: string;
  role: OrgRole;
}): Promise<AccessResult<{ members: AccessMember[] }>> {
