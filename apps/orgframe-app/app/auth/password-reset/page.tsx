...
export default async function ResetPage({
  ...
  const [query, sessionUser] = await Promise.all([searchParams, getSessionUser()]);
  const isUpdateMode = query.mode === "update";
  const prefilledEmail = sessionUser?.email ?? "";
        ...
        {isUpdateMode ? (
          ...
          <form action={requestPasswordResetAction} className="space-y-3">
            <FormField label="Email">
              <Input autoComplete="email" defaultValue={prefilledEmail} name="email" required type="email" />
            </FormField>
