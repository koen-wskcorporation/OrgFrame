...
set
  permissions = (
    ...
    from (
      select
        case permission
          when 'facilities.read' then 'spaces.read'
          when 'facilities.write' then 'spaces.write'
          else permission
        end as permission
      from unnest(coalesce(custom_role.permissions, '{}'::text[])) as permission
    ) mapped
  ),
  updated_at = now()
where coalesce(custom_role.permissions, '{}'::text[]) && array['facilities.read', 'facilities.write']::text[];
