...
begin
  ...
  into
    resolved_form_kind,
    resolved_settings
  from forms.org_forms form_definition
  ...
  from forms.org_form_submissions submission
...
drop trigger if exists org_form_submissions_enforce_cap on forms.org_form_submissions;
create trigger org_form_submissions_enforce_cap
  before insert on forms.org_form_submissions
  for each row
...
language plpgsql
security definer
set search_path = public, orgs, forms
as $$
declare
