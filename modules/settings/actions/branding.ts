...
export async function saveOrgBrandingAction(orgSlug: string, formData: FormData) {
  try {
    ...
    if (accent && !isValidHexColor(accent)) {
      redirect(`/${orgSlug}/workspace/settings/branding?error=invalid_accent`);
    }
    ...
    if (error) {
      redirect(`/${orgSlug}/workspace/settings/branding?error=save_failed`);
    }

    redirect(`/${orgSlug}/workspace/settings/branding?saved=1`);
  } catch (error) {
