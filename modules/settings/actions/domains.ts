...
export async function saveOrgCustomDomainAction(orgSlug: string, formData: FormData) {
  try {
    ...
    if (!parsed.success) {
      redirect(`/${orgSlug}/workspace/settings/domains?error=invalid_domain`);
    }
    ...
    revalidatePath(`/${orgSlug}/workspace/settings/domains`);
    redirect(`/${orgSlug}/workspace/settings/domains?saved=1`);
  } catch (error) {
    rethrowIfNavigationError(error);
    redirect(`/${orgSlug}/workspace/settings/domains?error=${toSaveErrorCode(error)}`);
  }
}
...
export async function verifyOrgCustomDomainAction(orgSlug: string) {
  try {
    ...
    if (!existing) {
      redirect(`/${orgSlug}/workspace/settings/domains?error=missing_domain`);
    }
    ...
    if (!result.verified) {
      redirect(`/${orgSlug}/workspace/settings/domains?error=verification_failed`);
    }

    redirect(`/${orgSlug}/workspace/settings/domains?verified=1`);
  } catch (error) {
    rethrowIfNavigationError(error);
    redirect(`/${orgSlug}/workspace/settings/domains?error=verification_failed`);
  }
...
export async function removeOrgCustomDomainAction(orgSlug: string) {
  try {
    ...
    revalidatePath(`/${orgSlug}/workspace/settings/domains`);
    redirect(`/${orgSlug}/workspace/settings/domains?removed=1`);
  } catch (error) {
    rethrowIfNavigationError(error);
    redirect(`/${orgSlug}/workspace/settings/domains?error=remove_failed`);
  }
}
